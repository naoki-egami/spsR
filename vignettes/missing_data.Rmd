...
imputed_data <- impute_var(data = sps_country_data, 
                           id_site = 'country', 
                           id_year = 'year', 
                           id_impute = c('e_p_polity', 'NY.GDP.MKTP.CD'),
                           method = "amelia")
```
...
* `id_time`: A unique identifier for time index. A column name in `data`. If `NULL`, values will be merged onto all years in `basedata`. If unspecified, it assumes `data` is cross-sectional.

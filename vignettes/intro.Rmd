...
For example, we might want to select at least one site that have `Age` larger than `1` (i.e., 1 standard deviation as we standardized variables above). We can incorporate this information as follows. 

```{r}
st_age <- stratify_sps(X = X_Imm, 
                       num_site = list("at least", 1), 
                       condition = list("Age", "larger than or equal to", 1))
```
...
st_edu <- stratify_sps(X = X_Imm, 
                       num_site = list("at most", 2), 
                       condition = list("Education", "between", c(-0.5, 0.5)))
```
...
for(v in 1:7){
  st_large[[v]] <- stratify_sps(X = X_Imm, num_site = list("at least", 1), condition = list(colnames(X_Imm)[v], "larger than or equal to", 0.5))
  st_small[[v]] <- stratify_sps(X = X_Imm, num_site = list("at least", 1), condition = list(colnames(X_Imm)[v], "smaller than or equal to", -0.5))
}
```

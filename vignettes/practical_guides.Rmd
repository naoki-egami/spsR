...
When `sps()` is run on large data set, it may take a long time to solve the optimization problem. For example, selecting four out of 50 possible sites requires generating weights for 3,921,225 unique combinations of sites. To reduce the computational time, users may use [Gurobi Optimizer](https://www.gurobi.com/), which solves optimization problems much faster, by asserting "GUROBI" under the `solver` argument in `sps()`. However, the use of Gurobi Optimizer requires a license registration and a software installation. For an instruction on how to register and download Gurobi Optimizer, please see [Improve Computational Efficiency](https://naokiegami.com/spsR/articles/comp_efficiency.html).

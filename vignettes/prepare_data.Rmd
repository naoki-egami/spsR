...
kbl(codebook,
  ...
  kable_styling(font_size = 11, bootstrap_options = c('hover', 'condensed')) %>%
...
```{r}
search_result <- search_var(keyword = 'international migrant') %>% mutate_if(is.character, ~gsub('\\$', ' dollar', .x))
```
```{r, echo = F}
kbl(search_result %>% mutate_all(~gsub('\\\\|\t|\n', '', .x)),
...
You can also merge your own dataset using the function ``merge_data``, as long as it has a country variable. If your dataset does not have an year variable, ignore the `yearvar` argument and we'll merge the dataset to all years. For a country variable, if you provide an ISO3 code, we will merge your dataset using the ISO3 code via an exact match. If you provide a country name instead, we will first merge via an exact match and do a fuzzymatch for the remaining unmatched countries (using `fastLink` R package). For example:

```{r, echo = F}
naumann <- as.data.frame(X_Imm) %>% rownames_to_column() %>% select(-GDP, -Unemp., -Immigration, -rowname) %>% rename_all(tolower) 
naumann$cntry <- c("Netherlands","Sweden","Norway","France","Germany","Bellgium","Denmark","Finland","Ireland","Slovenia","Spain","Switzerland","Austria","Czech","United Kingdom")
naumann$yr    <- 2015
naumann %<>% select(cntry, yr, everything())
...
* `match`: results of the match for each row. `exact` and `fastLink` 

### Descriptive Summary

``desc_stat`` function returns simple descriptive statistics, number of missing countries in each year, and correlation matrix. Highly correlated variables mean that they are capturing similar cross-country contexts, so you might want to drop one of them in the SPS. Character and factor variables are automatically binarized to obtain descriptive statistics.
...
For your selected countries, you can also use the function ``create_map`` to create a world plot to show heterogeneity in selected covariates. For example,

```{r, fig.fullwidth = TRUE, fig.width = 9, fig.height = 5}
map <- create_map(data = sps_data1, 
                  region = c('Europe & Central Asia', 'Latin America & Caribbean', 'North America'),
                  countryvar = 'iso3', 
...
Enamorado, Ted, Ben Fifield, and Kosuke Imai. 2017. fastLink: Fast Probabilistic Record Linkage with Missing Data. Version 0.6. https://github.com/kosukeimai/fastLink.

Enamorado, Ted, Benjamin Fifield, and Kosuke Imai. 2019. “Using a Probabilistic Model to Assist Merging of Large-Scale Administrative Records.” American Political Science Review 113 (2): 353–71.

...
In R package `spsRdata`, we provide a country-level data set (`sps_country_data_full`) comprised of 21 variables for 179 countries between 2010 and 2022. It contains a wide range of variables commonly used in the social sciences: they represent political, economic, and social characteristics of countries. These variables are collected from the following sources. 

* [Varieties of Democracy (V-Dem) project](https://v-dem.net/data/the-v-dem-dataset)^[V-Dem data set collects variables from a variety of sources including United Nations, Quality of Government, Clio Infra, and Polity5 Project, and currently includes the most comprehensive political indicators.]
...
`sps_country_data_full` includes the following variables.
...
In our example, we are interested in 7 country-level variables discussed in the original paper: GDP, unemployment rates, size of migrant population, general support for immigration, the proportion of female, the mean age, and the mean education. We illustrate how to collect these variables step by step. 

We are interested in 15 European countries and variables measured in 2015 (the year in which most of the site-level variables are available). If users are running experiments now, they can, of course, choose site-level variables measured in most recent years. 

```{r}
sps_country_data1 <- sps_country_data(year = 2015)
```

Two out of the 7 variables are already in our default data set: GDP (`NY.GDP.MKTP.CD`) and unemployment rate (`SL.UNEM.TOTL.ZS`). So we subset the data set to include only those variables we're interested in. 

```{r}
sps_country_data2 <- sps_country_data1[, c('country', 'iso3', 'year', 'NY.GDP.MKTP.CD', 'SL.UEM.TOTL.ZS')]
```
...
```{r}
sps_country_data3 <- merge_data(data = sps_country_data2, vars = c('SM.POP.TOTL.ZS'))
...
* `newdata`: A data.frame that contains new variables to be merged on. Must be `NULL` if used to merge variables come from V-Dem or World Bank.
* `vars`: A vector with one or more variable names that should be merged onto `data`. Must be specified if `newdata` is `NULL` (use ``search_var()`` to search for new variables).
* `id_site`: A country variable name in both `data` and `newdata`. Ignored if `newdata` is `NULL`.
* `id_year`: A year variable name in both `data` and `newdata`. If `NULL`, values will be merged onto all years in `basedata`. Ignored if `newdata` is `NULL`.
* `...`: Arguments passed on to `fuzzymatch::stringdist_join()`. Ignored if `newdata` is `NULL`.

Users may use the arguments `data` and `vars` only to add variables from V-Dem or World Bank data set, as was done with `SM.POP.TOTL.ZS`. Otherwise, users may assert additional arguments---i.e., `newdata`, `id_site`, and `id_year`---to merge their own dataset. 

```{r}
sps_country_data4 <- merge_data(data = sps_country_data3, newdata = ess_data, id_site = 'country')
```
...
With a full set of site-level variables, users can run simple descriptive analyses to examine distribution, missing values, and correlations. The function ``desc_stat`` returns simple descriptive statistics and number of missing entries in each year in the data set. Character and factor variables are automatically binarized to obtain descriptive statistics:
...
#### <span style="color:#3366CC">**3.5 \ Clean Data**</span> {#clean}

There you have it! The data set now includes the site-level variables we want to diversify in SPS. We now subset the final data set to the target population (i.e., 15 European countries in our example), rename the variable names to be more intelligible, and move onto implementing SPS via `spsR`.

```{r}
row.names(sps_country_data4) <- sps_country_data4$iso3
X_Imm <- sps_country_data4[sps_country_data4$iso3 %in% ess_data$iso3, c("NY.GDP.MKTP.CD", "SL.UEM.TOTL.ZS", "SM.POP.TOTL.ZS", "female", "age", "education", "imm_s_avg")]
names(X_Imm) <- c('GDP', 'Unemployment', 'Immigration', 'Female', 'Age', 'Education', 'Immig_Support')
head(X_Imm, n = 3)
```

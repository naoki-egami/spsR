...
* `iso3`: Logical `TRUE` or `FALSE` indicating whther the values in `id_country` is an ISO3 country code. If `TRUE`, variables in `newdata` is merged onto `basedata` using the ISO3 code. If `FALSE`, then `newdata` is merged using an exact match first, followed by a fuzzy match. 
...
The function ``desc_stat`` returns simple descriptive statistics, number of missing countries in each year, and correlation matrix:
...
head(desc$miss, n = 3)
```
A list object `desc` returns descriptive summary of each variable in the dataset, whereas `miss` returns a total counts of missing values (i.e., number of missing countries) in each year in the dataset. Character and factor variables are automatically binarized to obtain descriptive statistics.

```{r}
round(desc$corr[1:3, 1:3], 3)
```

A list object `corr` returns correlation matrix for all pairs of variables in the dataset. Highly correlated variables mean that they are capturing similar cross-country contexts, so you might want to consider dropping one of them in the SPS. 

...
Users can check the code book using `data(sps_country_codebook)`. 

```{r}
data(sps_country_codebook)
sps_country_codebook[sps_country_codebook$Variable %in% c("country", "year", "NY.GDP.MKTP.CD", "SP.POP.TOTL"), ]
```
...
```{r}
country_use <- c("Austria", "Belgium", "Czechia", "Denmark", "Finland", "France", "Germany", "Ireland",
                 "Netherlands", "Norway", "Slovenia", "Spain", "Sweden", "Switzerland", "United Kingdom")

sps_country_data1 <- sps_country_data[sps_country_data$year == 2015 & sps_country_data$country %in% country_use,]
```
...
head(ess_data, n = 3)
```
First, it is strongly recommended to standardize country names using ISO3 code. Users can do this very easily with R package `countrycode`.
...
* `id_year`: A unique identifier for time index. A column name in both `data` and `newdata`. If `NULL`, the same values in `newdata` will be merged onto all years in `data`. Ignored if `newdata` is `NULL`.
...
Finally, we clean data for function `sps()`. First, we need to have informative row names for the data. Second, we only keep relevant variables and rename them. 

```{r}
row.names(sps_country_data4) <- sps_country_data4$country
X_Imm <- sps_country_data4[, c("NY.GDP.MKTP.CD", "SL.UEM.TOTL.ZS", "SM.POP.TOTL.ZS", "female", "age", "education", "imm_s_avg")]
colnames(X_Imm) <- c('GDP', 'Unemployment', 'Immigration', 'Female', 'Age', 'Education', 'Immig_Support')
head(X_Imm, n = 3)
```
